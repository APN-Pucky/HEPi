...
def mass_and_ratio_plot(dl,li,p,scale=False,pdf=False,combined=False,cont = False,figsize=(6,4),plot_data=True,fill=True,**kwargs):
    global fig, axs
    if not cont:
    ...
    if combined:
        ...
        for i in [0,1]:
            ...
            if p == "invariant_mass":
                ...
                if i == 1:
                    plot(dl,  "invariant_mass","nlo_plus_nll_over_nlo",**kinv, interpolate=True,plot_data=False,fill=False,xaxis="$M$ [GeV]",yaxis="Ratio",**kwargs,data_color='0',label="(nlo+nll)/nlo")
            else:
