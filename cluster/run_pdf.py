        ...
        for p in ppps:
            ...
            if not run_plot:
                ...
                for l,n in pdfs:
                    ...
                    hepi.vplot(hepi.get_mass(dp[n],p)[mask],dp[n]["NLO_PLUS_NLL"][mask]/plot.unv(dp[pdfs[0][1]]["NLO_PLUS_NLL"][nmask]),yaxis="Ratio",fill=False,interpolate=True,plot_data=False,label=n.replace('_','').replace('as','').upper(), xaxis="$m_{"+hepi.get_name(p) + "}$ [GeV]",logy=False,data_color=cs[ci],lpos=1)
                    hepi.vplot(hepi.get_mass(dp[n],p)[mask],dp[n]["NLO_PLUS_NLL_PDF"][mask]/plot.unv(dp[pdfs[0][1]]["NLO_PLUS_NLL"][nmask]),yaxis="Ratio",fill=True,interpolate=False,plot_data=False,label="", xaxis="$m_{"+hepi.get_name(p) + "}$ [GeV]",logy=False,data_color=cs[ci],lpos=1)
                    ci = ci +1
                ...
                for l,n in pdfs:
                    ...
                    hepi.vplot(hepi.get_mass(dp[n],p)[mask],dp[n]["NLO"][mask]/plot.unv(dp[pdfs[0][1]]["NLO"][nmask]),yaxis="Ratio",fill=False,interpolate=True,plot_data=False,label=n.replace('_','').replace('as','').upper(), xaxis="$m_{"+hepi.get_name(p) + "}$ [GeV]",logy=False,data_color=cs[ci])
                    hepi.vplot(hepi.get_mass(dp[n],p)[mask],dp[n]["NLO_PDF"][mask]/plot.unv(dp[pdfs[0][1]]["NLO"][nmask]),yaxis="Ratio",fill=True,interpolate=False,plot_data=False,label="", xaxis="$m_{"+hepi.get_name(p) + "}$ [GeV]",logy=False,data_color=cs[ci])
                    ci = ci +1
                hepi.title(plt.gca(),li[0],scenario=scenario[0:8]+ " " + scenario[8])
