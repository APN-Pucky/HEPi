...
for run_plot in [True, ]:
    for lo_pdf,nlo_pdf in [("CT18NLO","CT18NLO"), ("MSHT20nlo_as118","MSHT20nlo_as118"), ("NNPDF40_lo_as_01180","NNPDF40_nlo_as_01180")]:
        for ps in [2000002, 1000002]:
            p = 1000022
            i = hepi.Input(hepi.Order.NLO_PLUS_NLL, 13000, ps, p, "scenarioB.in", lo_pdf, nlo_pdf, 1., 1.,precision=0.001,max_iters=50)

            li = hepi.mass_scan([i], p, np.linspace(900, 2000, 32))
