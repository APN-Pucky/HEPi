...
input.set_pre("while [ $(squeue -u " + user+ " | grep wc -l) >= 1998 ]; do sleep 60; done && sbatch --job-name=" +os.path.basename(sys.argv[0]) +
...
def wait():
    ...
    while(len(output.split('\n')) > 1):
        bashCommand = "squeue -u " + user + " | grep " + os.path.basename(sys.argv[0])
